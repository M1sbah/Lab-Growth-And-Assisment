...
router.get("/",async(req,res) =>{
    if(req.session.email && await Faculty.findOne({
        email:req.session.email
    })){
        let userMail=await Faculty.findOne({
            email:req.session.email
        });
        let pid=await AssignQues.distinct("paperID");
        // let qD=await AssignQues.find({});
        res.status(201).render('dashboard',{title:'Dashboard',data:userMail,assignments:pid});
...
router.post('/register',async(req,res) =>{
    ...
    try {
        let user_exist =await Faculty.findOne({email:email});
        if(user_exist){
            let usr={
                exist:true
            }
            return res.status(409).render('register',{data:usr});
        }
